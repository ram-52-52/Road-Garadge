...
const MechanicEarnings = () => {
        ...
        const fetchFinancials = async () => {
            try {
                ...
                if (profileRes.success) {
                    const garageId = profileRes.data._id;
                    const [earningsRes, jobsRes] = await Promise.all([
                        getGarageEarnings(garageId),
                        getGarageJobs(garageId)
                    ]);
    ...
    return (
        <div className="min-h-full bg-slate-50 p-4 xs:p-6 sm:p-8 space-y-8 xs:space-y-12 animate-in fade-in duration-1000">
            {/* Treasury Header HUD */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 bg-white p-6 xs:p-10 rounded-[2rem] xs:rounded-[3.5rem] border border-slate-100 shadow-sm relative overflow-hidden group">
                <div className="absolute top-0 right-0 w-64 h-64 bg-blue-500/5 rounded-full blur-[80px]" />
                <div className="relative z-10 space-y-2 xs:space-y-4 text-center sm:text-left">
                    <div className="inline-flex items-center gap-2 px-3 py-1 bg-blue-50 border border-blue-100 rounded-full text-[7px] xs:text-[9px] font-black uppercase tracking-[0.2em] xs:tracking-[0.4em] text-blue-600 leading-none">
                        Financial Ledger Active
                    </div>
                    <h2 className="text-lg xs:text-4xl font-black text-slate-950 tracking-tighter italic uppercase leading-none">Partner Treasury</h2>
                </div>
                <button 
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 xs:gap-8">
                ...
                <div className="lg:col-span-2 bg-slate-950 rounded-[2.5rem] xs:rounded-[4rem] p-8 xs:p-10 sm:p-12 text-white relative overflow-hidden shadow-2xl">
                    ...
                    <div className="relative z-10 space-y-8 xs:space-y-12">
                        <div className="flex flex-col xs:flex-row items-center justify-between gap-6 text-center xs:text-left">
                            <div className="space-y-1 xs:space-y-2">
                                <p className="text-[8px] xs:text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] xs:tracking-[0.4em]">Settled Balance (Total)</p>
                                <h3 className="text-xl xs:text-4xl sm:text-6xl font-black italic tracking-tighter uppercase whitespace-nowrap leading-none">
                                    {stats ? formatCurrency(stats.revenue_total) : '₹ 0'}
                                </h3>
                            </div>
                        ...
                        <div className="flex flex-row justify-center xs:justify-start gap-8 xs:gap-12 pt-6 xs:pt-8 border-t border-white/5">
                            <div className="space-y-1 xs:space-y-2 text-center xs:text-left">
                                <span className="text-[8px] xs:text-[10px] font-black uppercase tracking-widest text-slate-500 italic">Today Output</span>
                                <p className="text-xl xs:text-2xl font-black text-emerald-400 italic tracking-tighter leading-none">
                            ...
                            <div className="space-y-1 xs:space-y-2 text-center xs:text-left">
                                ...
                                <p className="text-xl xs:text-2xl font-black text-blue-400 italic tracking-tighter leading-none">
                                    {stats ? formatCurrency(stats.revenue_week) : '₹ 0'}
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <div className="space-y-6 xs:space-y-8">
                ...
                <div className="bg-white rounded-[2.5rem] xs:rounded-[4rem] border border-slate-100 shadow-sm overflow-hidden pb-6 xs:pb-8 flex flex-col">
                    <div className="overflow-x-auto custom-scrollbar">
                        <table className="w-full text-left min-w-[300px] lg:min-w-0">
                            ...
                            <tbody className="divide-y divide-slate-50 flex flex-col lg:table-row-group">
                                {history.length > 0 ? (
                                    history.map((trx) => (
                                        <tr key={trx._id} className="hover:bg-slate-50/50 transition duration-300 group flex flex-col lg:table-row p-6 xs:p-10 border-b lg:border-none">
                                            <td className="lg:px-12 lg:py-8 mb-4 lg:mb-0">
                                                ...
                                                <div className="flex flex-col">
                                                    <span className="text-xs xs:text-sm font-black text-slate-950 tracking-tighter italic uppercase leading-none">{trx._id.slice(-8).toUpperCase()}</span>
                                                    <span className="text-[8px] xs:text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">
                                            ...
                                            <td className="lg:px-12 lg:py-8 mb-4 lg:mb-0">
                                                ...
                                                <span className="text-xs xs:text-sm font-black text-slate-700 italic uppercase leading-tight line-clamp-1">{trx.service_type}</span>
                                            </td>
                                            <td className="lg:px-12 lg:py-8 mb-4 lg:mb-0">
                                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block lg:hidden mb-1">Amount Archetype</span>
                                                <span className="text-base xs:text-2xl lg:text-lg font-black text-slate-950 italic tracking-tighter leading-none">{formatCurrency(trx.amount || 500)}</span>
                                            </td>
                                            <td className="lg:px-12 lg:py-8 mb-4 lg:mb-0">
                                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block lg:hidden mb-2">Handshake Status</span>
                                                <div className={`inline-flex items-center gap-2 px-3 xs:px-4 py-1.5 rounded-full text-[8px] xs:text-[10px] font-black uppercase tracking-widest border ${trx.status === 'COMPLETED' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-amber-50 text-amber-600 border-amber-100'}`}>
                                    ...
                                    <tr>
                                        <td colSpan={5} className="py-20 text-center">
                                            <p className="text-xs font-black text-slate-400 uppercase tracking-widest">No financial artifacts found in historical grid.</p>
                                        </td>
                                    </tr>
                                )}
                            </tbody>
